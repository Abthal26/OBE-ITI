        ...
        <form action="{{ route('admin.courses.store') }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" id="code" name="code" value="{{ old('code') }}" class="form-input @error('code') border-red-500 @enderror" placeholder="IF201" required>
                    @error('code')
                ...
                <div>
                    ...
                    <input type="number" id="sks" name="sks" value="{{ old('sks', 3) }}" min="1" max="6" class="form-input" required>
                </div>
            </div>
            
            <div>
                <label for="name" class="form-label">Nama Mata Kuliah <span class="text-red-500">*</span></label>
                <input type="text" id="name" name="name" value="{{ old('name') }}" class="form-input" placeholder="Algoritma dan Struktur Data" required>
            </div>
            ...
            <div class="grid grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="number" id="semester" name="semester" value="{{ old('semester', 1) }}" min="1" max="8" class="form-input" required>
                </div>
                <div>
                    <label for="academic_year" class="form-label">Tahun Akademik <span class="text-red-500">*</span></label>
                    <input type="text" id="academic_year" name="academic_year" value="{{ old('academic_year', date('Y').'/'.(date('Y')+1)) }}" class="form-input" placeholder="2023/2024" required>
                </div>
                <div>
                    ...
                    <select id="academic_period" name="academic_period" class="form-input" required>
                        <option value="ganjil" {{ old('academic_period') == 'ganjil' ? 'selected' : '' }}>Ganjil</option>
                        <option value="genap" {{ old('academic_period') == 'genap' ? 'selected' : '' }}>Genap</option>
                    </select>
                </div>
            </div>
