...
@section('subtitle', $student->nim . ' - ' . $student->name)
        ...
        <form action="{{ route('admin.students.update', $student) }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" id="nim" name="nim" value="{{ old('nim', $student->nim) }}" class="form-input @error('nim') border-red-500 @enderror" required>
                    @error('nim')
                ...
                <div>
                    ...
                    <input type="number" id="angkatan" name="angkatan" value="{{ old('angkatan', $student->angkatan) }}" min="2000" max="2100" class="form-input" required>
                </div>
            </div>
            
            <div>
                <label for="name" class="form-label">Nama Lengkap <span class="text-red-500">*</span></label>
                <input type="text" id="name" name="name" value="{{ old('name', $student->name) }}" class="form-input @error('name') border-red-500 @enderror" required>
                @error('name')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
